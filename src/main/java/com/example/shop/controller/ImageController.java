...
public class ImageController {
        ...
        @PostMapping(path = "/{productId}/add", consumes = MediaType.APPLICATION_OCTET_STREAM_VALUE)
        ...
        @PatchMapping(path = "/{id}", consumes = MediaType.APPLICATION_OCTET_STREAM_VALUE)
        @Operation(summary = "Обновление изображения")
        @ApiResponses({
                        @ApiResponse(responseCode = "404", description = "Изображение не найдено"),
                        @ApiResponse(responseCode = "415", description = "Неподдерживаемый тип данных"),
        })
        public ResponseEntity<Images> updateImage(@PathVariable UUID id, @RequestBody byte[] byteArray) {
                return ResponseEntity.ok(imageService.updateImage(id, byteArray));
        }
